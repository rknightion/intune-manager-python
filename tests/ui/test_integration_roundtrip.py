...
async def test_auth_graph_service_repository_round_trip(
    ...
    factory = GraphClientFactory(
        ...
        GraphClientConfig(scopes=["https://graph.microsoft.com/.default"], enable_telemetry=False),
    )
    try:
        with respx.mock as route:
            route.get(
                "https://graph.microsoft.com/v1.0/deviceManagement/managedDevices",
            ).mock(return_value=httpx.Response(200, json={"value": [d.to_graph() for d in devices]}))
            ...
            controller.register_callbacks(
                refreshed=lambda items, from_cache: refreshed_events.append((list(items), from_cache)),
            )
            ...
            assert "Surface" in first_device
